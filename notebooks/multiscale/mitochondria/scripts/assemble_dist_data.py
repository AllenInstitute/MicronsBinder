"""
Assembling data to analyze distance-dependence
"""
import numpy as np
import pandas as pd
...
def assemble_dist_data(cellid, mitodf, refine=False, no_diam=False):
    segskel = skel.read_smoothed_skel(cellid)
    nodelbls = skel.read_smoothed_complbls(cellid)
    if refine:
    ...
    for (mitoid, nodeids) in mito_to_node.items():
        ...
        for nodeid in nodeids:
            ...
            if not no_diam:
                # current skeleton radius is more like a diameter,
                # and scaling this to um
                t_diams.append(segskel.vertex_properties["rs"][nodeid] / 1000.)

    data_dict = dict(mitoids=t_mitoids, mitovols=t_mitovols,
                     nodeids=t_nodeids, nodelbls=t_nodelbls,
                     nodedists=t_nodedists, cellid=t_cellids)
